...
import { BookOpen, Key, Lightbulb, AlertTriangle, ArrowRight, Code2, Waypoints } from 'lucide-react';
import { Link } from 'react-router-dom';
import SEO from "../../../../components/SEO";
import CodeBlock from "../../../../components/CodeBlock";
import JsReferenceSimulator from "../../../../simulators/web/js/JsReferenceSimulator";
...
const ReferenceTypes = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="Reference Types | JavaScript Course"
                description="Understand JavaScript reference types (objects, arrays, functions) and how memory address sharing works."
                keywords="javascript reference types, javascript heap memory, deep clone, shallow clone, mutable objects"
                url="/webdevelopment/javascript/reference-types"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }} className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-amber-500/10 border border-amber-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(251,191,36,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 2.3: Reference Types</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-300 via-yellow-500 to-orange-500">Reference</span> Types.
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        Master pointer logic and resolve the notorious "Shared Reference Problem" in memory.
                    </p>
                </motion.div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                ...
                <Section title="2.3 Reference Data Types" icon={Waypoints} id="reference-types">
                    <p className="text-lg sm:text-xl font-medium text-white leading-relaxed mb-10 border-l-4 border-amber-500 pl-6 bg-black/20 py-4 rounded-r-2xl">
                        While primitives store their actual value in memory, reference types (like Objects, Arrays, and Functions) store a memory address that points to where the complex data is actually held in the heap.
                    </p>

                    <div className="pt-8 border-t border-gray-800">
                        <h3 className="text-2xl sm:text-3xl font-extrabold text-rose-400 mb-6">
                            The Shared Reference Problem
                        </h3>
                        <p className="text-gray-300 leading-relaxed mb-8">
                            Because two variables can hold the same memory address, mutating the data through one variable immediately affects the other. 
                        </p>
                        
                        <CodeBlock 
                            title="shared-reference.js"
                            language="javascript"
                            code={`const originalArray = [1, 2, 3];
const copiedArray = originalArray; // Copies the address, not the array!

copiedArray.push(4);
console.log(originalArray); // [1, 2, 3, 4] — The original mutated too!`}
                        />
                    </div>

                    <HighlightBox title="Pro Tip" type="tip">
                        Shallow clones (spread, <code>Object.assign</code>) copy only one level deep. If your object contains nested objects, their references are still shared. For true isolation, you must perform a deep clone (e.g., using <code>structuredClone()</code>).
                    </HighlightBox>

                    <div className="mt-16 mb-8 pt-8 border-t border-gray-800/60">
                        <h4 className="text-2xl sm:text-3xl font-black text-white mb-6">Interactive Memory Lab</h4>
                        <p className="text-gray-400 mb-8 font-medium">Click through the simulator to witness how the stack and the heap handle variables during assignment, mutation, and cloning.</p>
                        <JsReferenceSimulator />
                    </div>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/primitives" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">Primitives</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/dynamic-typing" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-amber-500/50 hover:shadow-[0_0_30px_rgba(251,191,36,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">Dynamic Typing</span>
                            </div>
