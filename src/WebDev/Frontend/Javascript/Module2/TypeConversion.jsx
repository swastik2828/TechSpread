...
import { BookOpen, Key, Lightbulb, AlertTriangle, ArrowRight, Code2, ArrowLeftRight } from 'lucide-react';
...
const TypeConversion = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="Type Conversion & Coercion | JavaScript Course"
                description="Understand explicit conversion and implicit coercion in JavaScript, evaluating truthy and falsy values."
                keywords="javascript type conversion, implicit coercion, explicit conversion javascript"
                url="/webdevelopment/javascript/type-conversion"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }} className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-amber-500/10 border border-amber-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(251,191,36,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 2.5: Type Conversion</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        Type <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-300 via-yellow-500 to-orange-500">Conversion.</span>
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        Bridging the gap between types automatically vs intentionally.
                    </p>
                </motion.div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                ...
                <Section title="2.5 Type Conversion & Coercion" icon={ArrowLeftRight} id="type-conversion">
                    <p className="text-lg sm:text-xl font-medium text-white mb-10 p-6 bg-[#0d1117] border border-gray-800 rounded-2xl shadow-inner">
                        JavaScript frequently needs to bridge the gap between different types, and it does so through two mechanisms: explicit conversion (which you initiate deliberately) and implicit coercion (which JavaScript performs automatically).
                    </p>

                    <h3 className="text-2xl sm:text-3xl font-extrabold text-blue-400 mt-12 sm:mt-16 mb-6 sm:mb-8 border-b border-gray-800 pb-3 sm:pb-4">
                        Explicit Conversion
                    </h3>
                    <p>
                        Explicit conversion uses built-in functions to intentionally change a value's type. Because you have chosen to perform the conversion, your intention is clear to any reader. The primary explicit conversion functions are <code>Number()</code>, <code>String()</code>, <code>Boolean()</code>, <code>parseInt()</code>, and <code>parseFloat()</code>. Each has a defined, predictable behaviour for every input type.
                    </p>

                    <h3 className="text-2xl sm:text-3xl font-extrabold text-rose-400 mt-12 sm:mt-16 mb-6 sm:mb-8 border-b border-gray-800 pb-3 sm:pb-4">
                        Implicit Coercion
                    </h3>
                    <p>
                        Implicit coercion is JavaScript's automatic type conversion, which occurs when operators are used with operands of mismatched types. The rules follow a specification algorithm and are consistently applied — but they are not always intuitive. The <code>+</code> operator with at least one string operand converts the other to a string and concatenates. Arithmetic operators other than <code>+</code> convert operands to numbers. Comparison operators with <code>==</code> perform a type-coercing comparison, following complex rules that produce surprising results. This is precisely why the community universally recommends using <code>===</code> (strict equality) instead.
                    </p>

                    <CodeBlock 
                        title="coercion-examples.js"
                        language="javascript"
...
null == undefined   // true`}
                    ...
                    <div className="mt-16 mb-8 pt-8 border-t border-gray-800/60">
                        <h4 className="text-2xl sm:text-3xl font-black text-white mb-6">Interactive Coercion Visualizer</h4>
                        <p className="text-gray-400 mb-8 font-medium">Compare the predictability of Explicit Conversion against the surprising behaviors of Implicit Coercion on the JavaScript engine.</p>
                        <JsTypeCoercionSimulator />
                    </div>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/dynamic-typing" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">Dynamic Typing</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/operators" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-amber-500/50 hover:shadow-[0_0_30px_rgba(251,191,36,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">Operators</span>
                            </div>
