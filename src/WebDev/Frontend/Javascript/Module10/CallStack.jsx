...
import { BookOpen, Key, Lightbulb, AlertTriangle, ArrowRight, Compass, Layers, Activity } from 'lucide-react';
import { Link } from 'react-router-dom';
import SEO from "../../../../components/SEO";
import CodeBlock from "../../../../components/CodeBlock";
import JsCallStackSimulator from "../../../../simulators/web/js/JsCallStackSimulator";
...
const CallStack = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="The Call Stack — Where Code Actually Runs | JavaScript Course"
                description="Understand LIFO execution context, stack frames, stack overflow limits, and why an empty stack triggers the Event Loop."
                keywords="javascript call stack, lifo, stack frame, execution context, stack overflow, call stack empty"
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 10.3: Event Loop & Concurrency</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        The Call Stack: <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-yellow-500">Execution</span>
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        The Call Stack is a LIFO (Last In, First Out) data structure that tracks which function is currently executing and which function called it.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                ...
                <Section title="How the Call Stack Works" icon={Layers} id="how-stack-works">
                    <p className="text-gray-300 leading-relaxed">
                        When you call a function, a <strong>stack frame</strong> is pushed onto the stack. This frame holds:
                    </p>
                    <ul className="list-disc pl-6 space-y-3 mt-4 font-medium">
                        <li>The function's local variables</li>
                        <li>The function's arguments</li>
                        <li>A return address (where to go when this function finishes)</li>
                    </ul>
                    <p className="text-gray-300 leading-relaxed mt-4">
                        When the function returns, its frame is <strong>popped</strong> off the stack.
                    </p>

                    <CodeBlock 
                        title="call-stack-demo.js"
                        language="javascript"
                        code={`function greet(name) {
  const message = sayHello(name);   // push sayHello
  console.log(message);             // push console.log, then pop
}                                   // pop greet
...
greet('Ada');                       // push greet`}
                    />

                    <h4 className="text-white font-bold mt-6 mb-2">Stack Progression timeline:</h4>
                    <pre className="p-4 rounded-xl bg-black border border-gray-800 text-xs font-mono text-gray-300">
{`[]
→ [greet]
→ [greet, sayHello]
→ [greet]           ← sayHello returned
→ [greet, console.log]
→ [greet]           ← console.log returned
→ []                ← greet returned`}
                    </pre>
                </Section>

                <Section title="Interactive Stack Visualizer" icon={Activity} id="visualizer-demo">
                    <HighlightBox title="Observe Memory & Frames" type="tip">
                        Click the Step Forward button in the simulator below to watch exactly how variables change within their execution contexts and how stack frames push/pop sequentially.
                    </HighlightBox>
                    <JsCallStackSimulator />
                </Section>

                <Section title="The Empty Call Stack Trigger" icon={AlertTriangle} id="empty-stack-trigger">
                    <HighlightBox title="The Golden Rule of the Event Loop" type="warn">
                        The Event Loop only intervenes <strong>when the call stack is completely empty</strong>. This is the trigger. If the stack is busy, no callbacks run — ever. No timer fires, no Promise resolves to your code. Everything waits.
                    </HighlightBox>
                    <p className="text-gray-300 leading-relaxed">
                        This leads to a classic problem called <strong>Blocking</strong>:
                    </p>
                    <CodeBlock 
                        title="blocking-loop.js"
                        language="javascript"
                        code={`// This synchronous loop freezes the page!
for (let i = 0; i < 1e9; i++) {
  // Call stack is occupied, page is locked, Event Loop is blocked
}`}
                    />
                </Section>

                <Section title="Stack Overflow" icon={AlertTriangle} id="stack-overflow">
                    <p className="text-gray-300 leading-relaxed">
                        If recursion goes too deep (thousands of nested calls), you get <code>RangeError: Maximum call stack size exceeded</code>. This is the stack's memory limit being hit — it has nothing to do with the Event Loop or timers.
                    </p>
                </Section>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/big-picture" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">10.2 The Big Picture</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/web-apis" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-yellow-500/50 hover:shadow-[0_0_30px_rgba(168,85,247,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">Web APIs Layer</span>
                            </div>
