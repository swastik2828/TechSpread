...
import { BookOpen, Key, Lightbulb, AlertTriangle, ArrowRight, Compass, Layers } from 'lucide-react';
...
const NodeDifferences = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="Node.js Differences: libuv and Extra Phases | JavaScript Course"
                description="Explore Node.js Event Loop architecture, standard libuv threads, Node's six distinct event loop phases, setImmediate vs setTimeout, and process.nextTick."
                keywords="nodejs event loop, libuv event loop, nodejs phases, process.nexttick, setimmediate vs settimeout, check phase"
                url="/webdevelopment/javascript/node-differences"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 10.11: Event Loop & Concurrency</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        Node.js vs Browser: <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-yellow-500">libuv Loop</span>
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        Node.js uses the same V8 engine as Google Chrome, but it swaps out the browser's Web APIs for libuv — a highly specialized C library that provides async thread pools and I/O.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                ...
                <Section title="Node's Six Loop Phases" icon={Layers} id="node-phases">
                    <p className="text-gray-300 leading-relaxed mb-6">
                        Node's event loop has 6 distinct phases executed in a strict chronological sequence on every tick:
                    </p>

                    <pre className="p-4 rounded-xl bg-black border border-gray-800 text-xs font-mono text-gray-300 leading-relaxed max-w-lg mx-auto">
{`   ┌──────────────────────────────┐
...
└──┤      close callbacks         │  e.g. socket.on('close', ...)
   └──────────────────────────────┘`}
                ...
                <Section title="process.nextTick() — Node's Microtask Upgrade" icon={BookOpen} id="next-tick">
                    <p className="text-gray-300 leading-relaxed">
                        <code>process.nextTick(fn)</code> is Node-specific and behaves like a high-priority microtask — it runs **before** Promise microtasks in the same drain phase.
                    </p>
                    <CodeBlock 
                        title="nexttick-demo.js"
                        language="javascript"
...
// Output: nextTick, promise, timeout or immediate (order varies)`}
                    />
                </Section>

                <Section title="setTimeout vs setImmediate" icon={Lightbulb} id="setimmediate-timing">
                    <HighlightBox title="Determinism inside callbacks" type="tip">
                        When called in the main thread outside of file I/O, the execution order of `setTimeout(fn, 0)` vs `setImmediate(fn)` is non-deterministic and depends on system process speeds.
                        <br /><br />
                        However, **inside an I/O callback**, <code>setImmediate</code> always runs first, because check phase occurs immediately after the poll phase has collected I/O events.
                    </HighlightBox>
                    <CodeBlock 
                        title="io-deterministic.js"
                        language="javascript"
...
fs.readFile(__filename, () => {
  ...
  setImmediate(() => console.log('immediate'));
  // Output is always: immediate, timeout
});`}
                    />
                </Section>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/rendering-pipeline" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">10.10 Render Pipeline</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/common-bugs" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-yellow-500/50 hover:shadow-[0_0_30px_rgba(168,85,247,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">Common Bugs</span>
                            </div>
