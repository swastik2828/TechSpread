...
const PerformancePatterns = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="Performance Patterns & Best Practices | JavaScript Course"
                description="Optimize asynchronous JavaScript code: main-thread yielding with setTimeout, multi-threading with Web Workers, event debouncing/throttling, and queueMicrotask."
                keywords="javascript performance patterns, yield main thread settimeout, web workers parallelism, debounce throttle event listeners, queueMicrotask"
                url="/webdevelopment/javascript/performance-patterns"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 10.13: Event Loop & Concurrency</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        Best Practices: <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-yellow-500">Performance</span>
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        Master the structural performance design patterns to maintain blistering fast, 60fps applications under synchronous computation workloads.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                ...
                <Section title="13.1 Yielding Main-Thread with setTimeout" icon={Clock} id="yielding-thread">
                    <p className="text-gray-300 leading-relaxed">
                        When executing CPU-bound calculations on huge arrays, partition the dataset into smaller chunks and yield back to the browser between iterations so layout and visual frames continue smoothly:
                    </p>
                    <CodeBlock 
                        title="yielding-chunks.js"
                        language="javascript"
                        code={`async function processLargeArray(items) {
  ...
  for (let i = 0; i < items.length; i += CHUNK_SIZE) {
    ...
    // Yield back to the browser event loop — allows paints
    await new Promise(resolve => setTimeout(resolve, 0));
                ...
                <Section title="13.2 True Parallelism with Web Workers" icon={Zap} id="web-workers">
                    <p className="text-gray-300 leading-relaxed">
                        For extremely high-intensity work (e.g. image processing, heavy encryption, parsing), move the computation entirely off the main thread into separate CPU contexts:
                    </p>
                    <CodeBlock 
                        title="main.js"
                        language="javascript"
...
self.onmessage = (e) => {
  ...
  self.postMessage(result);
                    ...
                    />
                    <HighlightBox title="Separate Threads!" type="key">
                        Web Workers run on separate operating system threads with their own distinct Event Loop, communicating back to the main thread via asynchronous message passing.
                    </HighlightBox>
                </Section>

                <Section title="13.3 Debouncing and Throttling Event Handlers" icon={Lightbulb} id="debounce-throttle">
                    <p className="text-gray-300 leading-relaxed">
                        User interaction events (scroll, mousemove, resize) fire thousands of times per second. Wrap listeners inside throttling or debouncing helpers to restrict callback queuing rates:
                    </p>
                    <div className="space-y-4">
                        <p className="text-xs font-bold uppercase text-yellow-400">Debounce: executes only after typing pauses for 300ms</p>
                        <CodeBlock 
                            title="debounce.js"
                            language="javascript"
...
input.addEventListener('input', debounce(handleSearch, 300));`}
                        />
                        <p className="text-xs font-bold uppercase text-yellow-400">Throttle: executes at most once every 100ms</p>
                        <CodeBlock 
                            title="throttle.js"
                            language="javascript"
...
window.addEventListener('scroll', throttle(handleScroll, 100));`}
                ...
                <Section title="13.4 queueMicrotask() for Fine-Grained Priority" icon={BookOpen} id="queuemicrotask">
                    <p className="text-gray-300 leading-relaxed">
                        <code>queueMicrotask(fn)</code> schedules something directly as a microtask without resolving a dummy Promise wrapper, providing slightly higher efficiency:
                    </p>
                    <CodeBlock 
                        title="queuemicrotask-demo.js"
                        language="javascript"
                        code={`// Equivalent to Promise.resolve().then(fn) but cleaner and slightly faster
queueMicrotask(() => {
  // executes immediately after currently running synchronous code
  flushPendingUpdates();
});`}
                    />
                </Section>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/common-bugs" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">10.12 Common Bugs</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/mental-models" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-yellow-500/50 hover:shadow-[0_0_30px_rgba(168,85,247,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">Cheat Sheets</span>
                            </div>
