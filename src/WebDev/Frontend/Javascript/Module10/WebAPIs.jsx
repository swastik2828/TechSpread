...
import { BookOpen, Key, Lightbulb, AlertTriangle, ArrowRight, Compass, Cpu, Network } from 'lucide-react';
...
const WebAPIs = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="The Web APIs Layer — Borrowed Threads | JavaScript Course"
                description="Understand how the browser's Web APIs layer uses background OS threads to handle timeouts, network requests, and DOM events concurrently without freezing JavaScript."
                keywords="web apis, asynchronous javascript, single threaded, parallel execution, multi-threading browser, settimeout, fetch"
                url="/webdevelopment/javascript/web-apis"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 10.4: Event Loop & Concurrency</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        Web APIs: <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-yellow-500">Parallelism</span>
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        JavaScript is single-threaded, but your computer is not. The browser (or Node.js runtime) has access to OS threads and can use them to do work in parallel with your JS code.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                <p className="text-lg sm:text-xl font-medium text-white leading-relaxed mb-8 border-l-4 border-yellow-500 pl-6 italic">
                    "When you delegate tasks like networking and timers to Web APIs, the main thread stays entirely free to keep executing your synchronous JS code."
                </p>

                <Section title="Delegating to the Environment" icon={Cpu} id="delegating-apis">
                    <p className="text-gray-300 leading-relaxed">
                        When you write asynchrony, you call APIs provided by the container:
                    </p>
                    <ul className="list-disc pl-6 space-y-3 mt-4 text-gray-300">
                        <li><code>setTimeout(fn, 1000)</code> — the browser starts a timer on a background thread.</li>
                        <li><code>fetch(url)</code> — the browser issues a network request using OS networking modules.</li>
                        <li><code>element.addEventListener('click', fn)</code> — the browser listens for OS-level input events.</li>
                        <li><code>fs.readFile(path, fn)</code> (Node.js) — libuv uses a C++ thread pool to read files in the background.</li>
                    </ul>

                    <HighlightBox title="JavaScript Doesn't Wait!" type="key">
                        Your JavaScript code registers the callback function and immediately moves on to the next line. The browser does all the heavy waiting elsewhere in the background.
                    </HighlightBox>
                </Section>

                <Section title="Source of All Asynchrony" icon={Network} id="source-async">
                    <p className="text-gray-300 leading-relaxed">
                        When a background operation completes, the background thread moves the callback to the Task Queues.
                    </p>
                    <HighlightBox title="The Core Insight" type="tip">
                        The Web APIs layer is the source of <strong>all</strong> asynchrony in JavaScript. Promises, async/await, and the Event Loop are just sophisticated machinery designed to manage callbacks returning from this layer.
                    </HighlightBox>
                </Section>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/concurrency-call-stack" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">10.3 The Call Stack</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/task-queues" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-yellow-500/50 hover:shadow-[0_0_30px_rgba(168,85,247,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">The Task Queues</span>
                            </div>
