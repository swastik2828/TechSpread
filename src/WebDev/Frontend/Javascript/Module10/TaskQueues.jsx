...
import { BookOpen, Key, Lightbulb, AlertTriangle, ArrowRight, Compass, Clock, Activity } from 'lucide-react';
...
const TaskQueues = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="The Task Queues — The Waiting Room | JavaScript Course"
                description="Explore the Job Queue (Microtasks) and Callback Queue (Macrotasks), understand their scheduling priorities, and use the interactive priorities dashboard."
                keywords="microtask queue, macrotask queue, job queue, callback queue, task queue, javascript prioritization, javascript course"
                url="/webdevelopment/javascript/task-queues"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 10.5: Event Loop & Concurrency</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        Task Queues: <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-yellow-500">The Queues</span>
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        When an async operation completes and its callback is ready to run, the callback doesn't immediately jump onto the call stack. It waits in a queue.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                <p className="text-lg sm:text-xl font-medium text-white leading-relaxed mb-8 border-l-4 border-yellow-500 pl-6 italic">
                    "Two waiting rooms, two priorities: Promises command swift execution, while timers and user events yield politely."
                </p>

                <Section title="The Waiting Room Queues" icon={Clock} id="waiting-rooms">
                    <p className="text-gray-300 leading-relaxed text-base sm:text-lg">
                        JavaScript partitions callbacks into two queues with distinct execution priorities:
                    </p>

                    <div className="space-y-6 mt-6">
                        <div className="p-6 rounded-2xl bg-[#12141c] border border-yellow-500/20">
                            <h4 className="text-lg font-bold text-yellow-400 mb-2">5.1 The Microtask Queue (High Priority)</h4>
                            <p className="text-sm text-gray-300 leading-relaxed">
                                Also known as the <strong>Job Queue</strong> in ECMAScript specification.
                                <br />
                                <strong>What goes here:</strong> Promise `.then()` / `.catch()` / `.finally()` callbacks, `async` function resumptions (after an `await` checkpoint), `queueMicrotask(fn)` callbacks, and browser `MutationObserver` callbacks.
                                <br />
                                <strong>Draining Rule:</strong> The Event Loop drains the <em>entire</em> microtask queue before releasing control to anything else. If a microtask adds another microtask, that new microtask is executed immediately, too!
                            </p>
                        </div>

                        <div className="p-6 rounded-2xl bg-[#12141c] border border-sky-500/20">
                            <h4 className="text-lg font-bold text-sky-400 mb-2">5.2 The Macrotask Queue (Normal Priority)</h4>
                            <p className="text-sm text-gray-300 leading-relaxed">
                                Also called the <strong>Task Queue</strong> or <strong>Callback Queue</strong>.
                                <br />
                                <strong>What goes here:</strong> `setTimeout` and `setInterval` timers, I/O completions, DOM click/keydown event listeners, and `setImmediate` (Node.js).
                                <br />
                                <strong>Processing Rule:</strong> The Event Loop processes exactly <strong>ONE</strong> macrotask per tick. After executing it, it returns to drain all pending microtasks again.
                            </p>
                        </div>
                    </div>
                </Section>

                <Section title="Interactive Priority Dashboard" icon={Activity} id="interactive-priority-demo">
                    <HighlightBox title="Interactive Sandbox" type="tip">
                        Use the dashboard below to schedule different types of tasks (Promises vs setTimeouts). Click <strong>Tick Event Loop</strong> to watch the scheduler prioritize draining Microtasks completely before processing Macrotasks.
                    </HighlightBox>
                    <JsTaskPrioritySimulator />
                </Section>

                <Section title="Why Two Queues?" icon={Key} id="why-two">
                    <p className="text-gray-300 leading-relaxed">
                        Promises represent things that are already logically resolved (the value is available, the decision has been made). It would be strange and buggy if a <code>Promise.resolve().then(fn)</code> callback executed <em>after</em> a <code>setTimeout(fn, 0)</code> that was registered earlier in the code. Promises need reliable, immediate delivery. The Microtask Queue ensures this priority.
                    </p>
                </Section>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/web-apis" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">10.4 Web APIs Layer</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/event-loop-orchestrator" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-yellow-500/50 hover:shadow-[0_0_30px_rgba(168,85,247,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
