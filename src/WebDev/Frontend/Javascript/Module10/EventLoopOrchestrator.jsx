...
import { BookOpen, Key, Lightbulb, AlertTriangle, ArrowRight, Compass, Cpu, Activity } from 'lucide-react';
...
const EventLoopOrchestrator = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="The Event Loop — The Orchestrator | JavaScript Course"
                description="Understand the core Event Loop algorithm, its step-by-step loop logic in pseudocode, ticks, iterations, and run-to-completion semantics."
                keywords="event loop algorithm, event loop pseudocode, event loop tick, run to completion, single threaded javascript, event loop course"
                url="/webdevelopment/javascript/event-loop-orchestrator"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 10.6: Event Loop & Concurrency</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        The Event Loop: <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-yellow-500">Orchestrator</span>
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        The Event Loop is a conceptually simple algorithm running continuously in a infinite loop, monitoring the call stack and dispatching callbacks.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                <p className="text-lg sm:text-xl font-medium text-white leading-relaxed mb-8 border-l-4 border-yellow-500 pl-6 italic">
                    "Step by step, tick by tick, it keeps single-threaded JavaScript responsive to thousands of parallel network requests."
                </p>

                <Section title="The Event Loop Algorithm" icon={Cpu} id="algorithm-pseudocode">
                    <p className="text-gray-300 leading-relaxed">
                        Here is the exact logical loop the environment runs continuously behind the scenes:
                    </p>
                    ...
                    <CodeBlock 
                        ...
                        language="javascript"
                        code={`while (true) {
    // 1. Run the current task (initially: the script itself)
    runCurrentTask();
    ...
    // 3. Optionally: render (browser only, throttled to 60fps)
    if (renderingIsNeeded()) {
        runAnimationFrameCallbacks();
        render();
    }

    // 4. Pick ONE macrotask (if any)
    if (macrotaskQueue.isNotEmpty()) {
        currentTask = macrotaskQueue.dequeue();
    }

    // → loop back to step 1 with the new task
}`}
                    />
                    <p className="text-gray-300 leading-relaxed">
                        Each pass through this loop is called a <strong>tick</strong> or an <strong>iteration</strong>.
                    </p>
                </Section>

                <Section title="Event Loop Interactive Visualizer" icon={Activity} id="interactive-visualizer">
                    <HighlightBox title="Watch the ticks" type="tip">
                        Use the interactive player below to watch the Call Stack, Web APIs, and priority queues interact as they execute a baseline code block!
                    </HighlightBox>
                    <JsEventLoopSimulator />
                </Section>

                <Section title="Run-to-Completion Semantics" icon={Key} id="run-to-completion">
                    <p className="text-gray-300 leading-relaxed">
                        Importantly, <strong>a task runs to completion</strong>. Once a macrotask starts executing on the call stack, it runs until the stack is completely empty. The Event Loop cannot interrupt it or pause it mid-way.
                    </p>
                    <HighlightBox title="Why synchronous loops freeze the browser" type="warn">
                        This is why a long-running synchronous loop (e.g. <code>for (let i = 0; i &lt; 1e9; i++)</code>) freezes your browser window. No other task, render check, or button click event can run until that loop yields the call stack.
                    </HighlightBox>
                </Section>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/task-queues" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">10.5 The Task Queues</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/priority-system" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-yellow-500/50 hover:shadow-[0_0_30px_rgba(168,85,247,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">Priority System</span>
                            </div>
...
export default EventLoopOrchestrator;
