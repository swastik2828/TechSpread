...
import { Target, Key, Lightbulb, AlertTriangle, ArrowRight } from 'lucide-react';
...
const HighlightBox = ({ title, children, type = "info" }) => {
    const config = {
        ...
        note: { icon: Target, color: "text-sky-400", border: "border-sky-500/40", bg: "bg-gradient-to-br from-sky-900/40 to-blue-900/10", heading: "text-sky-400", shadow: "shadow-sky-500/10" },
    ...
    }[type] || { icon: Target, color: "text-sky-400", border: "border-sky-500/40", bg: "bg-gradient-to-br from-sky-900/40 to-blue-900/10", heading: "text-sky-400", shadow: "shadow-sky-500/10" };
...
const ExplicitBinding = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="Explicit Binding | JavaScript Course"
                description="Master Explicit Binding in JavaScript using call(), apply(), and bind() to force the 'this' keyword to point exactly where you want it."
                keywords="javascript explicit binding, call apply bind, javascript this context, force this keyword"
                url="/webdevelopment/javascript/explicit-binding"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 7.3: Explicit Binding</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-yellow-500">Explicit</span> Binding.
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        The "I'm Telling You" Rule: Taking manual control over `this`.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                ...
                <Section title="2. Explicit Binding" icon={Target} id="explicit-binding">
                    <p className="text-lg sm:text-xl font-medium text-white leading-relaxed">
                        Sometimes you don't want to leave things to chance. You want to explicitly tell JavaScript exactly what <code>this</code> should be. You do this using three built-in function methods: <code>.call()</code>, <code>.apply()</code>, and <code>.bind()</code>.
                    </p>
                    ...
                    <div className="space-y-8 bg-[#0d1117] p-6 sm:p-10 rounded-3xl border border-gray-800/60 shadow-inner mt-8">
                        ...
                        <ul className="space-y-4 text-gray-300">
                            <li><strong><code>call(thisArg, arg1, arg2)</code></strong>: Runs the function immediately. The first argument becomes <code>this</code>. The rest are passed to the function normally.</li>
                            <li><strong><code>apply(thisArg, [arg1, arg2])</code></strong>: Identical to call, but you pass the function's arguments as an array. <br/><em>(Memory trick: Call uses Commas, Apply uses an Array).</em></li>
                            <li><strong><code>bind(thisArg, arg1)</code></strong>: Does not run the function. Instead, it <strong>returns a brand-new copy</strong> of the function with <code>this</code> permanently locked to whatever you provided.</li>
                        </ul>
                    </div>
                    ...
                    <div className="mt-12">
                        <CodeBlock 
                            title="explicit.js"
                            language="javascript"
                            code={`function introduce(lang1, lang2) {
  console.log(\`I am \${this.name} and I write \${lang1} and \${lang2}.\`);
}

const dev = { name: "Sarah" };

// Explicitly forcing 'this' to be the 'dev' object
introduce.call(dev, "JavaScript", "Python"); 
// Output: "I am Sarah and I write JavaScript and Python."

introduce.apply(dev, ["C++", "Java"]);
// Output: "I am Sarah and I write C++ and Java."

// Bind creates a locked copy for later use
const boundIntroduce = introduce.bind(dev, "Rust", "Go");
boundIntroduce(); 
// Output: "I am Sarah and I write Rust and Go."`}
                        />
                    </div>
                </Section>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/new-binding" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">The new Binding</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/implicit-default-binding" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-yellow-500/50 hover:shadow-[0_0_30px_rgba(168,85,247,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">Implicit & Default Binding</span>
                            </div>
...
export default ExplicitBinding;
