...
import { Target, Key, Lightbulb, AlertTriangle, ArrowRight, Zap } from 'lucide-react';
...
const HighlightBox = ({ title, children, type = "info" }) => {
    const config = {
        ...
        note: { icon: Zap, color: "text-sky-400", border: "border-sky-500/40", bg: "bg-gradient-to-br from-sky-900/40 to-blue-900/10", heading: "text-sky-400", shadow: "shadow-sky-500/10" },
    ...
    }[type] || { icon: Zap, color: "text-sky-400", border: "border-sky-500/40", bg: "bg-gradient-to-br from-sky-900/40 to-blue-900/10", heading: "text-sky-400", shadow: "shadow-sky-500/10" };
...
const ArrowFunctionsThis = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="Arrow Functions and 'this' | JavaScript Course"
                description="Learn how arrow functions handle the 'this' keyword in JavaScript using Lexical Scoping."
                keywords="javascript arrow functions, arrow function this, lexical this, lexical scoping"
                url="/webdevelopment/javascript/arrow-functions-this"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 7.5: Arrow Functions & Lexical `this`</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-yellow-500">Arrow</span> Functions.
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        The Special Case: Why Arrow Functions completely ignore the 4 rules of `this`.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                ...
                <Section title="7.5 The Special Case: Arrow Functions" icon={Zap} id="arrow-functions">
                    <p className="text-lg sm:text-xl font-medium text-white leading-relaxed">
                        Arrow functions (<code>{`() => {}`}</code>) were introduced in ES6 specifically to fix the headache of losing <code>this</code> in callbacks.
                    </p>

                    <HighlightBox title="Lexical Scoping" type="key">
                        Arrow functions completely ignore the 4 rules we learned. They <strong>do not have their own this</strong>. Instead, they inherit <code>this</code> from the surrounding scope exactly where they were defined. This is called Lexical Scoping.
                        <br/><br/>
                        Think of an arrow function like a child looking for their keys. If they don't have them in their own pocket, they ask their parent.
                    </HighlightBox>
  ...
  start() {
    // ❌ A regular function would lose 'this' here
    // ✅ An arrow function remembers 'this' belongs to the Timer instance
    setInterval(() => {
      this.seconds++; 
...
myTimer.start(); // 1, 2, 3...`}
                    ...
                    <HighlightBox title="Cannot be explicitly bound" type="warn">
                        Note: Because arrow functions have their <code>this</code> hardcoded by their environment, you cannot use <code>.call()</code>, <code>.apply()</code>, or <code>.bind()</code> to change an arrow function's <code>this</code>.
                    </HighlightBox>
                </Section>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/implicit-default-binding" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">Implicit & Default Binding</span>
                            </div>
                        </Link>
                    </div>
                </div>
...
export default ArrowFunctionsThis;
