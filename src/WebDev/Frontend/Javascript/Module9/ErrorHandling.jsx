...
const ErrorHandling = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="Error Handling in Async Code | JavaScript Course"
                description="Learn best practices for handling errors in asynchronous JavaScript, including try/catch, unhandled rejections, and fetch issues."
                keywords="javascript error handling, async error handling, try catch, unhandled promise rejection, fetch api errors"
                url="/webdevelopment/javascript/error-handling"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 9.6: Asynchronous JS</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-yellow-500">Error Handling</span>
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        Preventing silent failures in asynchronous operations.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                ...
                <p className="text-lg sm:text-xl font-medium text-white leading-relaxed mb-8">
                    Handling errors correctly in async code is one of the areas where developers most commonly make mistakes. The rules are simple, but easy to forget — and forgetting them leads to <strong>silent failures</strong>, where your application breaks without telling you why.
                </p>

                <Section title="try/catch/finally" icon={Key} id="try-catch">
                    <p className="text-gray-300 leading-relaxed">
                        With async/await, you handle errors using the same <code>try/catch</code> blocks you'd use for synchronous code. This is a deliberate design benefit.
                    </p>
  ...
  try {
    ...
    hideLoadingSpinner(); // Always runs — success or failure
                    ...
                    <p className="text-gray-300 leading-relaxed mt-4">
                        The <code>finally</code> block is particularly valuable in async code. It runs regardless of whether the <code>try</code> block succeeded or the <code>catch</code> block ran. Without <code>finally</code>, you'd need to hide the loading spinner in both the success path and the failure path, which is error-prone.
                    </p>
                </Section>

                <Section title="A Critical Gotcha: fetch() Doesn't Reject on HTTP Errors" icon={ShieldAlert} id="fetch-errors">
                    <HighlightBox title="Common Bug" type="warn">
                        Developers assume that if <code>fetch()</code> didn't throw, the request was successful. <strong>This is wrong.</strong>
                    </HighlightBox>
                    <p className="text-gray-300 leading-relaxed mt-4">
                        <code>fetch()</code> only rejects when there is a <em>network-level failure</em> — no internet connection, DNS resolution failed, server is completely unreachable. If the server <em>responds</em> — even with a <code>404 Not Found</code> or <code>500 Internal Server Error</code> — <code>fetch()</code> considers that a successful communication and <strong>fulfills the Promise</strong>.
                    </p>
                    <p className="text-gray-300 leading-relaxed mt-4">
                        You must check <code>response.ok</code> manually:
                    </p>
                    <div className="mt-4 mb-6">
                        <CodeBlock 
                            title="check-ok.js"
  ...
  // Must check this manually! A 404 or 500 does NOT cause a rejection.
  if (!response.ok) {
  ...
  return response.json();
                ...
                <Section title="Layered Error Handling" icon={BookOpen} id="layered">
                    <p className="text-gray-300 leading-relaxed">
                        You don't always need to catch errors in the same function that makes the call. Different layers handle different kinds of errors:
                    </p>
                    <ul className="list-disc pl-6 space-y-3 mt-4 text-gray-300 font-medium">
                        <li><strong>Low-level functions</strong> (like an API wrapper) might catch network errors and rethrow them with more context, but let domain errors bubble up.</li>
                        <li><strong>Mid-level functions</strong> might catch domain-specific errors (like "user not found") and return a sensible default, letting unexpected errors propagate.</li>
                        <li><strong>High-level functions</strong> (like a page initialization) catch everything that slipped through and decide what the user should see.</li>
                    </ul>
                </Section>

                <Section title="Unhandled Promise Rejections" icon={AlertTriangle} id="unhandled">
                    <p className="text-gray-300 leading-relaxed">
                        If a Promise rejects and no <code>.catch()</code> or <code>try/catch</code> ever handles it, it becomes an <strong>unhandled Promise rejection</strong>. In browsers, this logs a warning to the console. In modern Node.js, it <strong>crashes the process</strong>.
                    </p>
                    <div className="mt-4 mb-6">
                        <CodeBlock 
                            title="unhandled.js"
                            language="javascript"
                            code={`// ❌ If fetchData() rejects, it goes unhandled
doWork();

// ✅ Always ensure rejections are handled
doWork().catch(err => handleError(err));`}
                        />
                    </div>
                </Section>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/async-await" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">Async / Await</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/sequential-vs-parallel" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-yellow-500/50 hover:shadow-[0_0_30px_rgba(168,85,247,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">Sequential vs Parallel</span>
                            </div>
