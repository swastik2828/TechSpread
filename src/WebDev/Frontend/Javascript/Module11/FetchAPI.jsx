...
import { BookOpen, Key, Lightbulb, AlertTriangle, ArrowRight, Compass, Code, Network, Globe } from 'lucide-react';
import { Link } from 'react-router-dom';
import SEO from "../../../../components/SEO";
import CodeBlock from "../../../../components/CodeBlock";
import JsFetchGotchaSimulator from '../../../../simulators/web/javascript/JsFetchGotchaSimulator';
...
const FetchAPI = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="The Fetch API | JavaScript Web APIs"
                description="Learn the modern fetch() API, how to handle HTTP errors properly, and build production-quality fetch wrappers."
                keywords="fetch, fetch API, response.ok, HTTP requests, javascript fetch, async await fetch"
                url="/webdevelopment/javascript/fetch-api"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(234,179,8,0.2)]">
                        ...
                        <span className="text-amber-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 11.2: Web APIs & Data Handling</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        The <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-yellow-500">Fetch API</span>
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        <code>fetch()</code> is the modern, built-in way to make HTTP requests from JavaScript. It replaced the older XMLHttpRequest (XHR) API and integrates naturally with Promises.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                ...
                <Section title="The Simplest Possible Fetch" icon={Code} id="simple-fetch">
                    <p className="text-gray-300 leading-relaxed">
                        Making a GET request requires just two awaits: one for the network response, and one to parse the JSON body.
                    </p>
                    <CodeBlock 
...
console.log(user.name); // "Alice"`} 
                        language="javascript" 
                    />
                    <p className="mt-4 text-gray-400 italic">Simple — but dangerously incomplete for real use. Here's why.</p>
                </Section>

                <Section title="The Critical Gotcha" icon={AlertTriangle} id="fetch-gotcha">
                    <HighlightBox title="Fetch Doesn't Throw on HTTP Errors" type="warn">
                        This is the most important thing to understand about fetch():

                        fetch() only rejects (throws) for network-level failures — like having no internet connection or a DNS lookup failing. 
                        
                        HTTP errors like 404 or 500 do NOT cause fetch() to throw. They come back as resolved Promises!
                    </HighlightBox>

                    <JsFetchGotchaSimulator />

                    <p className="text-gray-300 leading-relaxed mt-6">
                        The `Response` object has an `ok` property that is `true` for any 2xx status code and `false` for everything else. You <strong>must</strong> check it:
                    </p>
...
const user = await response.json();`} 
                ...
                <Section title="Reading the Response Body" icon={BookOpen} id="response-body">
                    <p className="text-gray-300 leading-relaxed">
                        A `Response` body can only be read <strong>once</strong>. Reading it consumes a stream under the hood. All reading methods return Promises.
                    </p>
                        ...
                        <table className="w-full text-left text-sm text-gray-400">
                            <thead className="text-xs text-gray-300 uppercase bg-gray-900/80 border-b border-gray-800">
                                <tr>
                                    ...
                                    <th className="px-6 py-4">Use when...</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr className="border-b border-gray-800/50"><td className="px-6 py-4 font-mono text-yellow-400">response.json()</td><td className="px-6 py-4">Parsed JS object</td><td className="px-6 py-4">Server returns JSON (most APIs)</td></tr>
                                <tr className="border-b border-gray-800/50"><td className="px-6 py-4 font-mono text-yellow-400">response.text()</td><td className="px-6 py-4">String</td><td className="px-6 py-4">Server returns HTML, plain text, XML</td></tr>
                                <tr className="border-b border-gray-800/50"><td className="px-6 py-4 font-mono text-yellow-400">response.blob()</td><td className="px-6 py-4">Blob</td><td className="px-6 py-4">Server returns a file (image, PDF)</td></tr>
                                <tr><td className="px-6 py-4 font-mono text-yellow-400">response.arrayBuffer()</td><td className="px-6 py-4">ArrayBuffer</td><td className="px-6 py-4">Binary data you need to process</td></tr>
                            </tbody>
                        </table>
                    </div>
                </Section>

                <Section title="Making POST, PUT, DELETE" icon={Globe} id="other-methods">
                    <p className="text-gray-300 leading-relaxed">
                        `fetch()` accepts a second argument — an options object — to configure the request type, headers, and body payload.
                    </p>
                    <CodeBlock 
                        code={`// POST: Create a new resource
  ...
  method: 'DELETE',
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/web-apis-foundation" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">11.1 How the Web Works</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/http-methods-rest" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-yellow-500/50 hover:shadow-[0_0_30px_rgba(234,179,8,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">11.3 HTTP Methods</span>
                            </div>
