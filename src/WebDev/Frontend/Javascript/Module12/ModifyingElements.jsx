...
import { PenTool, Key, Lightbulb, AlertTriangle, ArrowRight, Compass, Code, Layout, Layers } from 'lucide-react';
import { Link } from 'react-router-dom';
import SEO from "../../../../components/SEO";
import CodeBlock from "../../../../components/CodeBlock";
import JsDomModificationSimulator from '../../../../simulators/web/javascript/JsDomModificationSimulator';
...
const HighlightBox = ({ title, children, type = "info" }) => {
    const config = {
        ...
        note: { icon: PenTool, color: "text-sky-400", border: "border-sky-500/40", bg: "bg-gradient-to-br from-sky-900/40 to-blue-900/10", heading: "text-sky-400", shadow: "shadow-sky-500/10" },
    ...
    }[type] || { icon: PenTool, color: "text-sky-400", border: "border-sky-500/40", bg: "bg-gradient-to-br from-sky-900/40 to-blue-900/10", heading: "text-sky-400", shadow: "shadow-sky-500/10" };
...
const ModifyingElements = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="Creating and Modifying DOM Elements | JavaScript"
                description="Learn how to dynamically create, modify, and remove elements in the DOM using JavaScript. Understand textContent vs innerHTML and DocumentFragments."
                keywords="document.createElement, append, prepend, textContent, innerHTML, XSS, DocumentFragment, DOM manipulation"
                url="/webdevelopment/javascript/modifying-elements"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(59,130,246,0.2)]">
                        ...
                        <span className="text-yellow-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 12.3: DOM Manipulation</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600">Modifying</span> Elements
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        Building new interfaces on the fly means creating elements in memory, configuring them safely, and inserting them into the live tree.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                ...
                <Section title="Building Off-Tree" icon={Layout} id="create-element">
                    <p className="text-gray-300 leading-relaxed">
                        <code>document.createElement(tagName)</code> creates a brand-new element node that exists only in memory — it is not yet attached anywhere in the visible document.
                    </p>
                    <CodeBlock 
...
card.textContent = 'New card';`} 
                    ...
                    <p className="text-gray-300 leading-relaxed mt-4">
                        This detached state is a feature, not a limitation. You can set classes, attributes, content, and event listeners while it's still off-tree. None of that work triggers a visual update because the browser doesn't need to repaint anything that isn't on screen yet. Only when you insert it does the browser recalculate layout.
                    </p>
                </Section>

                <Section title="Inserting and Removing" icon={Layers} id="insertion">
                    <p className="text-gray-300 leading-relaxed">
                        The modern insertion methods read like plain English and all accept either a Node or a plain string (which gets automatically converted into a text node).
                    </p>
                    <CodeBlock 
                        code={`parent.append(child);        // Insert as the LAST child
parent.prepend(child);       // Insert as the FIRST child
sibling.before(newNode);     // Insert immediately BEFORE sibling
sibling.after(newNode);      // Insert immediately AFTER sibling
target.replaceWith(newNode); // Swap target out for newNode

// Removing is even simpler
element.remove();            // Detaches the element from its parent`} 
                        language="javascript" 
                    />
                </Section>

                <Section title="textContent vs innerHTML" icon={AlertTriangle} id="security-xss">
                    <p className="text-gray-300 leading-relaxed">
                        These two properties look interchangeable but behave very differently, and the difference is a genuine security boundary.
                    </p>

                    <HighlightBox title="The XSS Security Rule" type="warn">
                        If any part of a string comes from a user, a URL parameter, or an untrusted API, assigning it to `innerHTML` gives that source the ability to inject and run arbitrary scripts in your page (Cross-Site Scripting). 
                        
                        The rule is simple: <strong>Never assign unsanitized user input to innerHTML.</strong> Use `textContent` instead.
                    </HighlightBox>

                    <div className="mt-8">
                        <JsDomModificationSimulator />
                    </div>
                </Section>

                <Section title="Batching with DocumentFragment" icon={Code} id="document-fragment">
                    <p className="text-gray-300 leading-relaxed">
                        If you are inserting fifty elements at once, inserting them one at a time means the browser may recalculate layout fifty times. A <code>DocumentFragment</code> acts as an invisible, off-screen container to assemble nodes into first.
                    </p>
                    <CodeBlock 
                        code={`const fragment = document.createDocumentFragment();

// Build everything inside the invisible fragment
...
// ONE insertion, ONE layout recalculation
list.append(fragment);`} 
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/selecting-elements" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">12.2 Selecting Elements</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/data-attributes" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-yellow-500/50 hover:shadow-[0_0_30px_rgba(59,130,246,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">12.4 Data Attributes</span>
                            </div>
