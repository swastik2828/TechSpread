...
import { Database, Key, Lightbulb, AlertTriangle, ArrowRight, Compass, Settings } from 'lucide-react';
import { Link } from 'react-router-dom';
import SEO from "../../../../components/SEO";
import CodeBlock from "../../../../components/CodeBlock";
import JsDataAttributeSimulator from '../../../../simulators/web/javascript/JsDataAttributeSimulator';

// Reusing standard module layout components
...
const HighlightBox = ({ title, children, type = "info" }) => {
    const config = {
        ...
        note: { icon: Database, color: "text-sky-400", border: "border-sky-500/40", bg: "bg-gradient-to-br from-sky-900/40 to-blue-900/10", heading: "text-sky-400", shadow: "shadow-sky-500/10" },
    ...
    }[type] || { icon: Database, color: "text-sky-400", border: "border-sky-500/40", bg: "bg-gradient-to-br from-sky-900/40 to-blue-900/10", heading: "text-sky-400", shadow: "shadow-sky-500/10" };
...
const DataAttributes = () => {
    return (
        <article className="max-w-5xl mx-auto text-gray-300 flex flex-col gap-6 sm:gap-8 leading-loose tracking-wide font-sans pb-16 sm:pb-20 w-full px-4 sm:px-0">
            <SEO
                title="HTML Data Attributes in JavaScript | DOM Manipulation"
                description="Learn how to attach custom data fields directly to HTML elements using data-* attributes and read them efficiently via the element.dataset property."
                keywords="data attributes, element.dataset, HTML5 data-*, DOM custom data, parsefloat data attribute"
                url="/webdevelopment/javascript/data-attributes"
            />
            ...
            <header className="py-10 sm:py-16 md:py-24 text-center border-b border-white/10 mb-10 sm:mb-16 md:mb-20 relative overflow-hidden bg-gradient-to-b from-transparent to-black/20 rounded-b-[40px] w-full">
                ...
                <div className="w-full">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6 sm:mb-8 backdrop-blur-sm shadow-[0_0_20px_rgba(59,130,246,0.2)]">
                        ...
                        <span className="text-yellow-400 text-[10px] sm:text-xs md:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Module 12.4: DOM Manipulation</span>
                    </div>
                    ...
                    <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 tracking-tighter drop-shadow-2xl px-2 w-full break-words">
                        Data <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600">Attributes</span>
                    </h1>
                    
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed font-light px-4">
                        HTML5 introduced <code>data-*</code> attributes to officially sanction attaching custom data directly to an element, without abusing classes or IDs.
                    </p>
                </div>
            </header>
            ...
            <div className="prose prose-invert prose-base sm:prose-lg max-w-none text-gray-300 w-full border-box">
                ...
                <Section title="The Dataset Property" icon={Database} id="element-dataset">
                    <p className="text-gray-300 leading-relaxed">
                        JavaScript reads and writes <code>data-*</code> attributes through the <code>element.dataset</code> property. The browser automatically converts the HTML kebab-case names into JavaScript camelCase properties.
                    </p>
                    <CodeBlock 
                        code={`// HTML: 
// <button data-product-id="42" data-action="add-to-cart" data-price="29.99"> Add </button>
...
btn.addEventListener('click', () => {
  ...
  const price  = parseFloat(btn.dataset.price); // Convert string "29.99" to number
  ...
  addToCart(id, price);
                    ...
                    <div className="mt-12">
                        <h3 className="text-2xl font-bold text-white mb-6">Dataset Mapping Visualizer</h3>
                        <p className="text-gray-400 mb-6 text-sm">Click the buttons to see how HTML data attributes map into the JavaScript <code>dataset</code> object in memory.</p>
                        <JsDataAttributeSimulator />
                    </div>
                </Section>

                <Section title="Crucial Gotchas" icon={AlertTriangle} id="data-gotchas">
                    <HighlightBox title="Everything is a String" type="warn">
                        Two details catch people out here:
                        
                        1. <code>data-product-id</code> becomes <code>dataset.productId</code> — the hyphen disappears and the next letter capitalizes.
                        
                        2. <strong>Every value in dataset is a string.</strong> <code>data-price="29.99"</code> gives you the string <code>"29.99"</code>. If you need to do arithmetic, you must convert it yourself with <code>parseFloat</code> or <code>Number()</code>.
                    </HighlightBox>
                    <p className="text-gray-300 leading-relaxed mt-6">
                        Data attributes are best suited to small, simple values — flags, IDs, short labels. For structured data (arrays, nested objects), it's cleaner to keep that in a JavaScript object or framework state and use a <code>data-*</code> attribute only to link an element back to its corresponding JS state entry.
                    </p>
                </Section>
                ...
                <div className="mt-16 sm:mt-24 pt-8 sm:pt-12 border-t border-gray-800/60 text-center relative w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-6 sm:gap-4 w-full max-w-2xl mx-auto">
                        <Link to="/webdevelopment/javascript/modifying-elements" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-black/20 border border-gray-800 hover:border-gray-600 transition-all duration-300 w-full sm:w-auto self-start sm:self-auto order-2 sm:order-1">
                            ...
                            <div className="text-left w-full h-full">
                                ...
                                <span className="text-gray-300 font-bold text-xs sm:text-sm">12.3 Modifying Elements</span>
                            </div>
                        </Link>

                        <Link to="/webdevelopment/javascript/handling-events" className="group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#0a0c10] border border-gray-800 hover:border-yellow-500/50 hover:shadow-[0_0_30px_rgba(59,130,246,0.1)] transition-all duration-300 w-full sm:w-auto order-1 sm:order-2">
                            <div className="text-right w-full h-full">
                                ...
                                <span className="text-gray-200 font-bold text-xs sm:text-sm group-hover:text-white transition-colors">12.5 Handling Events</span>
                            </div>
